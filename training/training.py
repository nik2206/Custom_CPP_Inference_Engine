...
def main():
    ...
    model_json = {
        "conv_weight": state_dict["conv.weight"].cpu().numpy().flatten().tolist(),
        "conv_bias": state_dict["conv.bias"].cpu().numpy().flatten().tolist(),
        "fc_weight": state_dict["fc.weight"].cpu().numpy().flatten().tolist(),
        "fc_bias": state_dict["fc.bias"].cpu().numpy().flatten().tolist()
    }
    ...
    with torch.no_grad():
        ...
        logits = raw_output.cpu().numpy().flatten().tolist()
